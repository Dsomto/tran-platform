  ...
  SOC_ANALYSIS: [
    {
      ...
      objective: "Engineer a tested hunt pipeline for 5-8 million hostile-quality events: schema drift, duplicates, invalid timestamps, clock offsets, identity aliases, benign lookalikes, and three linked attack campaigns.",
      setup: ["Python 3.11+", "DuckDB with pytest", "16 GB RAM and 12 GB free disk", "Assigned V1-V6 archive with SHA-256 and three schema versions"],
      ...
      technicalChallenges: ["At least three schema versions per source and a malformed-row quarantine with reason codes.", "Variant-specific clock offsets must be inferred from cross-source anchors and corrected without reordering equal-time events.", "Campaign reconstruction must survive duplicate events, reused IPs, identity aliases, and approved activity sharing attacker-like indicators."],
      verificationTests: ["A clean staff runner ingests the full pack and executes all hunts in 20 minutes or less on 8 vCPU/16 GB RAM.", "A hidden 250,000-row shard must produce the exact staff campaign, false-positive, quarantine, and reconciliation counts.", "Two clean runs must produce identical normalized-table and result hashes; source-row accounting must equal manifest rows exactly."],
      proof: ["Every campaign edge resolves to two independent source locators.", "Automated tests cover schema drift, bad timestamps, duplicates, aliases, offsets, and empty inputs.", "The CLI rebuilds the database and exports all scored tables without manual SQL edits."],
    ...
    },
    {
      ...
      objective: "Deploy an isolated T-Pot sensor and build the collection, sessionization, clustering, payload handling, and detection pipeline around it; internet traffic volume is not a scoring input.",
      setup: ["Fresh supported minimal Linux VM", "8 GB RAM and 128 GB disk minimum for sensor; 16 GB/256 GB for hive", "Dedicated public IP; management ports restricted to the intern", "Official T-Pot installer run as a non-root user"],
      mission: ["Provision the sensor from a reproducible runbook and enforce ingress, egress, metadata, private-range, and management controls.", "Build a Python pipeline that exports T-Pot data, normalizes sessions, clusters infrastructure/credentials, hashes payloads, and emits STIX 2.1 plus Sigma or Suricata content.", "Replay the sealed protocol/session fixture through the pipeline and prove stable output independently of live collection volume.", "Quarantine every binary by hash and metadata only; never execute captured payloads."],
      ...
      verificationTests: ["All isolation rows pass from external, management, and sensor viewpoints before public exposure.", "A hidden sealed replay must produce exact session, cluster, payload, and detection counts.", "Destroy and redeploy must restore collection and the expected test alert within 45 minutes using the submitted runbook."],
      ...
      gates: ["Any reachable management service from an unapproved source fails the project.", "Any payload execution fails safety.", "Failure of sealed replay or clean redeploy fails technical completion."],
    ...
    },
    {
      ...
      mission: ["Define user, finance, engineering, server, management, guest, and DMZ zones entirely as code with deterministic addressing and routing.", "Implement stateful least privilege, administrative paths, NAT, DNS/NTP, centralized logs, and a mirrored sensor path.", "Write positive, negative, return-path, spoofing, management, and logging tests for the published matrix.", "Inject three supplied configuration faults, identify each from packets/logs, and restore the passing baseline through version-controlled changes."],
    ...
    },
    {
      ...
      objective: "Solve a sealed multi-host DFIR case from disk, memory, EVTX, email, identity, PCAP, and proxy evidence; recover the exfiltrated dataset and ship reproducible timeline and detection code.",
      setup: ["Volatility 3", "Wireshark and Zeek", "oletools", "Assigned sealed incident package", "40 GB free disk and 16 GB RAM recommended"],
  ...
  ETHICAL_HACKING: [
    {
      ...
      duration: "8 days", difficulty: 4, revision: "One revision", defense: "Artifact check",
      objective: "Engineer a resumable, scope-safe recon platform, pass an unreleased target fixture, discover the hosted variant's hidden service, and obtain a distinct target-issued foothold flag.",
      setup: ["Kali or Ubuntu VM", "Python 3.11 with pytest", "subfinder, httpx, naabu, nuclei, and nmap adapters", "Programme VPN access to the assigned hosted target"],
      ...
      verificationTests: ["The published unit/integration suite plus 20 hidden parser and scope fixtures must pass.", "Against a hidden hosted target, service recall must be at least 90 percent with zero out-of-scope packets and no more than the published request budget.", "With one external tool unavailable and one run interrupted, the engine must finish through fallback/resume and produce the same normalized result hash."],
      proof: ["Git history shows modules, tests, failures, and fixes rather than one generated commit.", "Packet capture proves zero decoy traffic and adherence to the rate budget.", "The distinct private foothold flag, raw response chain, and normalized record all reconcile."],
      ...
      gates: ["Only the assigned hosted target is in scope.", "The room evidence marker is not foothold proof; only the distinct target-issued flag counts.", "No auto-exploitation in the recon engine.", "A wrapper that ignores errors or only concatenates outputs is capped."],
    ...
    },
    {
      ...
      duration: "9 days", difficulty: 4, revision: "One revision", defense: "Artifact check",
      objective: "Write a reliable, non-Metasploit foothold-to-root exploit chain for an assigned offline VM, patch both root causes, and prove exploit and negative retest from clean snapshots.",
      setup: ["Assigned one of four calibrated offline VM images", "Kali attacker on host-only network", "Snapshot target before testing", "No walkthrough for the assigned box during the assessment window"],
    ...
    },
    {
      ...
      duration: "10 days", difficulty: 5, revision: "One revision", defense: "Artifact check",
      objective: "Compromise a staff-mutated CloudGoat-derived AWS range containing trust-policy, policy-version, permission-boundary, and workload-role edges; then implement and test least-privilege remediation.",
    ...
    },
    {
      ...
      setup: ["Assigned estate bundle: web, API, and infrastructure host", "Dedicated host-only network", "Signed rules of engagement and test window", "Clean snapshots for original and patched releases"],
