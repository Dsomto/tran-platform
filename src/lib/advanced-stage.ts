...
export const ADVANCED_PROJECTS: Record<AdvancedTrack, AdvancedProject[]> = {
  SOC_ANALYSIS: [
    {
      ...
      deliverables: ["hunt-engine/", "tests/", "queries/", "hunt-investigation-report.pdf", "benchmark.json", "normalized-timeline.csv", "campaign-graph.json", "tp-fp-table.csv", "data-quality-register.csv", "evidence-index.csv", "manifest.sha256"],
    ...
    },
    {
      ...
      objective: "Build portable detection-as-code for twelve ATT&CK behaviors from the signed replay, then pass attacks, mutations, benign lookalikes, telemetry failures, and deterministic clean runs.",
      setup: ["Python 3.11+ standard-library route", "Signed Windows replay and source manifest", "4 GB RAM and 2 GB free disk", "Optional Wazuh/Sysmon compatibility adapter for work already started"],
      mission: ["Verify and account for the immutable replay before deriving events.", "Execute twelve assigned behaviors plus twenty-four benign controls and preserve source/normalized/decision triples.", "Build an automated replay/regression harness and semantic detection tests; tune field logic, thresholds, relationships, and time windows.", "Measure behavior when telemetry is missing, a binary is renamed, a command is encoded differently, or timing changes."],
      ...
      verificationTests: ["All twelve canonical attack fixtures and all twenty-four benign fixtures must produce the published expected verdicts.", "At least six of eight hidden attack mutations must alert with zero alerts across twelve hidden benign fixtures.", "Two clean replay runs must complete unattended and produce identical result hashes."],
    ...
    },
    {
      ...
      deliverables: ["incident-report.pdf", "executive-brief.pdf", "timeline-builder/", "timeline.csv", "ioc-set.csv", "recovered/", "detections/", "tests/", "queries/", "evidence-index.csv", "manifest.sha256"],
  ...
  ETHICAL_HACKING: [
    ...
    {
      stage: "STAGE_8", slug: "stage-8", number: 4,
      title: "Break and Repair the Directory Control Plane",
      duration: ADVANCED_STAGE_WINDOW_LABEL, difficulty: 5, revision: "No revision", defense: "Recorded defense",
      objective: "Model a candidate-bound directory control plane, validate two independent proof paths, automate one path reliably, and prove detection plus remediation without infrastructure-dependent scoring.",
      setup: ["Python 3.11+ portable directory range", "Private candidate JSON from the stage room", "4 GB RAM and 2 GB free disk", "Optional GOAD-Light compatibility adapter for work already started"],
      mission: ["Generate and health-check the assigned offline range and preserve its hashes.", "Build an effective-rights graph; validate at least eight edges from primary records and reject the planted stale edge.", "Prove two independent proof paths, execute one through candidate-authored automation, and obey the proof limit.", "Write two event-sequence detections, remediate both paths, and rerun negative attack and legitimate-access tests."],
      ...
      verificationTests: ["The two private proof values and all directly validated edge IDs match the staff variant key.", "The automated path succeeds three of three times from a clean generated state and leaves health plus cleanup assertions green.", "After remediation, both path tests fail at their intended edges and both detections alert on positive replay fixtures but not benign controls."],
      ...
      resources: [...common(8), { label: "Project brief", href: p(8, "eh", "brief.md"), description: "Portable range, path requirements, and defense.", kind: "brief" }, { label: "Candidate variant JSON", href: "/api/advanced-stage/variant?stage=STAGE_8", description: "Your authenticated marker-bound offline range inputs.", kind: "artifact" }, { label: "Public AD tests", href: p(8, "eh", "public-tests.json"), description: "Exact edge validation, stale-path, cleanup, patch, and detection assertions.", kind: "artifact" }, { label: "Provisioning checklist", href: p(8, "eh", "provisioning-checklist.md"), description: "Portable range integrity and clean-state checks.", kind: "template" }, { label: "Credential handling", href: p(8, "eh", "credential-handling.md"), description: "Redaction and disposal contract.", kind: "template" }],
    },
    {
      stage: "STAGE_9", slug: "stage-9", number: 5,
      ...
      deliverables: ["engagement-suite/", "rules-of-engagement.pdf", "vapt-report.pdf", "executive-summary.pdf", "evidence/", "finding-tests/", "chain-runner/", "vulnerable-results.xml", "patched-results.xml", "retest-matrix.csv", "manifest.sha256"],
  ...
  GRC: [
    ...
    {
      stage: "STAGE_8", slug: "stage-8", number: 4,
      title: "Hardening Compiler and Quantified Risk",
      duration: ADVANCED_STAGE_WINDOW_LABEL, difficulty: 5, revision: "No revision", defense: "Recorded defense",
      objective: "Build a portable hardening-plan compiler with idempotent apply, rollback, security and service tests, then generate a tested quantitative treatment portfolio.",
      setup: ["Python 3.11+ portable Linux configuration sandbox", "No privileged host changes or VM required", "4 GB RAM and 1 GB free disk", "Optional Ansible/OpenSCAP compatibility adapter for work already started"],
      mission: ["Baseline and inventory the immutable sandbox and service contract.", "Implement at least eight scored controls through a declarative compiler with preconditions, validation, rollback, and no manual changes.", "Prove zero-change second apply, rollback, reapply, security delta, and service preservation.", "Write a risk-model program that consumes the supplied scan/assets, runs range and sensitivity calculations, and selects exactly three treatments under the variant budget."],
      technicalChallenges: ["The compiler's second apply must report zero changes and rollback must restore the baseline service/config hashes.", "One scanner finding is a false positive and one remediation conflicts with the declared service until correctly parameterized.", "The risk model must separate asset criticality, control effectiveness, loss ranges, dependencies, and residual risk without embedding finding IDs."],
      verificationTests: ["A clean sandbox must pass baseline, apply, second-run-idempotence, service, rollback, and reapply stages unattended.", "At least eight expected security deltas occur with zero service regressions; the false positive is not remediated as a real defect.", "The risk program must match published calculations and a hidden budget/asset fixture; recorded defense implements and tests one staff-selected submitted control."],
      proof: ["Raw before/after states, compiler results, config hashes, and service test XML.", "Zero-change second apply and successful rollback/reapply transcript.", "Tested risk-model outputs and exactly three funded treatments."],
      deliverables: ["control-compiler/", "control-schema/", "tests/", "rollback/", "before/", "after/", "security-results.json", "service-results.xml", "idempotence.json", "risk-model/", "risk-register.csv", "investment-memo.pdf", "video-url.txt", "manifest.sha256"],
      gates: ["No revision.", "Manual hardening or a non-idempotent compiler cannot pass.", "Any failed service or rollback test fails technical completion.", "Do not claim CIS certification from automated scan output."],
    ...
    },
    {
      ...
      deliverables: ["breach-engine/", "rules/", "schemas/", "tests/", "variant-snapshots/", "deadline-ledger.csv", "population-calculation.csv", "work-items.json", "regulator-notifications/", "subject-notice.md", "board-memo.pdf", "30-60-90-roadmap.csv", "manifest.sha256"],
