...
const GUIDANCE: Record<AdvancedTrack, Record<ProjectNumber, GuidanceSeed>> = {
  SOC_ANALYSIS: {
    ...
    4: {
      simpleTerms: "You're given a signed replay of Windows security activity and asked to build a portable detection engine for twelve attack techniques. You must prove it catches changed versions of the attacks without flagging normal activity, explain every failure state, and reproduce the same results twice. Existing Wazuh work still counts through an adapter, but no heavy lab is mandatory.",
      prerequisites: [
        "Parse versioned Windows and Sysmon-style event records without changing source evidence.",
        "Build a command-line test harness with deterministic machine-readable output.",
      ...
      rubric: [
        { label: "Release integrity and source accounting", weight: 10 },
        { label: "Normalization and provenance", weight: 15 },
        { label: "Detection logic", weight: 25 },
        { label: "Adversary and benign testing", weight: 25 },
        { label: "Deterministic automation", weight: 15 },
        { label: "Recorded defense", weight: 10 },
      ],
      environment: {
        hardware: "Portable scored route: Python 3.11+, 4 GB RAM, and 2 GB free disk. No VM, Docker, or live endpoint is required.",
        cost: "No paid infrastructure or product is required. Wazuh, Windows, Sysmon, and Atomic are optional compatibility evidence only.",
        fallback: "The signed replay is the controlling route. Existing full-lab work must be exported through the same portable event, verdict, and regression contract.",
      },
    },
    5: {
  ...
  ETHICAL_HACKING: {
    ...
    4: {
      simpleTerms: "You're given a candidate-bound offline model of a Windows directory with hidden permission weaknesses. You build the effective-rights graph, validate its raw records, discover two proof paths, automate one reliably, detect the decisive events, and repair both paths. It runs locally without GOAD or virtual machines.",
      prerequisites: [
        "Understand Active Directory authentication, groups, ACLs, delegation, Kerberos, and common privilege edges.",
        "Parse directory objects, memberships, ACLs, SPNs, sessions, and event records.",
        "Use candidate credentials and proof values without exposing them in reports.",
      ],
      glossary: [
        ...
        { term: "Portable range", meaning: "The candidate-bound offline directory control plane used as the scored baseline." },
      ],
      rubric: [
        { label: "Release integrity and scope", weight: 10 },
        { label: "Directory ingestion and effective rights", weight: 20 },
        { label: "Path discovery and direct validation", weight: 25 },
        { label: "Reliable automation and cleanup", weight: 20 },
        { label: "Detection and remediation", weight: 15 },
        { label: "Recorded defense", weight: 10 },
      ],
      environment: {
        hardware: "Portable scored route: Python 3.11+, 4 GB RAM, and 2 GB free disk. No hypervisor or live directory is required.",
        cost: "No paid directory, cloud service, VPN, GOAD deployment, or programme-hosted range is required.",
        fallback: "Existing GOAD work remains admissible through the portable graph and test contract; the issued offline range controls scoring.",
      },
    },
    5: {
  ...
  GRC: {
    ...
    4: {
      simpleTerms: "You are given a portable model of an insecure Linux host. You build a compiler that turns security controls into ordered changes, applies them only inside the sandbox, proves a second run changes nothing, rolls everything back, and keeps the application working. You also build a tested risk optimizer that selects exactly three treatments under a private budget.",
      prerequisites: [
        "Read and safely transform configuration files beneath a supplied target root.",
        "Design declarative controls, preconditions, rollback, and service acceptance tests.",
      ...
      glossary: [
        ...
        { term: "Security delta", meaning: "The explained difference between comparable before-and-after control test results." },
        { term: "Residual annual loss", meaning: "The modeled loss remaining after a treatment changes likelihood or impact." },
      ],
      rubric: [
        { label: "Release integrity and baseline model", weight: 10 },
        { label: "Control compiler and rollback", weight: 25 },
        { label: "Security and service delta", weight: 20 },
        { label: "Risk analysis", weight: 25 },
        { label: "Investment and deferral judgment", weight: 10 },
        { label: "Recorded defense", weight: 10 },
      ],
      environment: {
        hardware: "Portable scored route: Python 3.11+, 4 GB RAM, and 1 GB free disk. No VM or privileged host change is required.",
        cost: "No paid cloud, scanner, VM, Ansible, Lynis, or OpenSCAP installation is required.",
        fallback: "Existing VM or Ansible work remains admissible through the portable plan, state, rollback, and test contract; the supplied sandbox controls scoring.",
      },
    },
    5: {
