...
- Build `make ingest`, `make analyze`, and `make test` targets. The analysis
  pipeline must ingest both a live T-Pot export and the sealed fallback through
...
- Use the sealed programme fallback only after documenting a quiet sensor or a
  platform outage and receiving approval.
...
- A hidden sealed replay produces the exact session, cluster, payload, and
  detection counts in the staff oracle.
- Staff destroy and redeploy the sensor from the runbook. Collection and the
...
- **You receive:** the signed sealed replay, public session fixtures, isolation matrix, candidate marker, and a programme-authorized sensor address.
